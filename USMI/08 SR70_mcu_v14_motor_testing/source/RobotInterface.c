...
{
	m->m_state.status = ROBOT_OFF;
	m->m_state.command = ROBOT_OFF;
	for (int i=0;i<2;i++)
	{
...
void GPIO_init(RobotInterface * m)
{
	...
	GPIO_PinInit(Board_GPIO_DIR2_M1,Board_DIR_M1_2, &m->M1_D1_config);

	GPIO_PinInit(Board_GPIO_DIR_M2,Board_DIR_M2_1, &m->M2_D1_config);
	GPIO_PinInit(Board_GPIO_DIR2_M2,Board_DIR_M2_2, &m->M2_D1_config);

	GPIO_PinInit(Board_GPIO_DIR_M3,Board_DIR_M3_1, &m->M3_D1_config);
	GPIO_PinInit(Board_GPIO_DIR2_M3,Board_DIR_M3_2, &m->M3_D1_config);
...
void MotorTesting2(RobotInterface * m)
{
	...
	//M1
			...
			if (m->ADC_data[0] >=ADC_mid+ADC_zero_tol)
				{
					// CW
					Sig1=(unsigned int)(((double)(m->ADC_data[0]-ADC_mid)/ADC_mid)*99);
					GPIO_WritePinOutput(Board_GPIO_DIR_M1,Board_DIR_M1_1,1);
				...
				if(m->ADC_data[0] <ADC_mid-ADC_zero_tol)
				{	//CCW
					Sig1=(unsigned int)(((double)(ADC_mid-m->ADC_data[0])/ADC_mid)*99);
					GPIO_WritePinOutput(Board_GPIO_DIR_M1,Board_DIR_M1_1,0);
				...
				{
					// CW
					Sig2=(unsigned int)(((double)(m->ADC_data[1]-ADC_mid)/ADC_mid)*99);
					//PRINTF("sig3 %d \r\n",Sig3);
					GPIO_WritePinOutput(Board_GPIO_DIR_M2,Board_DIR_M2_1,1);
				...
				if(m->ADC_data[0] <ADC_mid-ADC_zero_tol)
				{	//CCW
					Sig2=(unsigned int)(((double)(ADC_mid-m->ADC_data[1])/ADC_mid)*99);
					GPIO_WritePinOutput(Board_GPIO_DIR_M2,Board_DIR_M2_1,0);
		 ...
		 // Motor3 by SBtn1,2
				...
				FTM_UpdatePwmDutycycle(BOARD_FTM_BASEADDR1, BOARD_FTM_CHANNEL1, kFTM_CenterAlignedPwm, (20U));

				FTM_UpdatePwmDutycycle(BOARD_FTM_BASEADDR2, BOARD_FTM_CHANNEL2, kFTM_CenterAlignedPwm, (20U));

				FTM_UpdatePwmDutycycle(BOARD_FTM_BASEADDR3, BOARD_FTM_CHANNEL3, kFTM_CenterAlignedPwm, (20U));*/
...
void ProcessingCommand(RobotInterface * m)
{
	...
	switch(this.command)
	{	// MCUOFF NOT IMPLEMENTED IN THIS STAGE
		...
		case ROBOT_OFF: switch(this.status){
									case ROBOT_MCUOFF: 	return;
									case ROBOT_OFF:		this.status = ROBOT_OFF; 	return;
									case ROBOT_STOP:	this.status = ROBOT_OFF;    return;
									case ROBOT_READY:	this.status = ROBOT_OFF;	return;
									case ROBOT_MOVE:	this.status = ROBOT_OFF;	return;
													}
		case ROBOT_STOP: switch(this.status){
									case ROBOT_MCUOFF: 	return;
									case ROBOT_OFF:		return;
									case ROBOT_STOP:	this.status = ROBOT_STOP; 	return;
									case ROBOT_READY:	this.status = ROBOT_STOP; 	return;
									case ROBOT_MOVE:	this.status = ROBOT_STOP;	return;
													}
		case ROBOT_READY: switch(this.status){
									case ROBOT_MCUOFF: 	return;
									case ROBOT_OFF:		return;
									case ROBOT_STOP:	this.status = ROBOT_READY;	return;
									case ROBOT_READY:	this.status = ROBOT_READY;	return;
									case ROBOT_MOVE:	/*output(0)*/
														this.status = ROBOT_READY;	return;
													}
		case ROBOT_MOVE: switch(this.status){
									case ROBOT_MCUOFF: 	return;
									case ROBOT_OFF:		return;
									case ROBOT_STOP:	/* error, Cant move*/
														this.command = ROBOT_STOP;	return;
									case ROBOT_READY:	/*run*/
														if(!1){this.command = ROBOT_READY;}
														this.command = ROBOT_MOVE;
														return;
									case ROBOT_MOVE:	/*msg "in motion"*/
														return;
													}
	}
}
void MotorTestingLiability(RobotInterface * m, int dir, unsigned int Sigs)
{
